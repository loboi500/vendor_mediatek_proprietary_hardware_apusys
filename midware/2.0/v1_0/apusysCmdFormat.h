...
#define SIZE_SUBGRAPH_SCOFS_ELEMENT sizeof(TYPE_SUBGRAPH_SCOFS_ELEMENT)
#define SIZE_CMD_SUCCESSOR_ELEMENT sizeof(TYPE_CMD_SUCCESSOR_ELEMENT)
#define SIZE_CMD_PREDECCESSOR_CMNT_ELEMENT sizeof(TYPE_CMD_PREDECCESSOR_CMNT_ELEMENT)

/* flag bitmap of apusys cmd */
enum {
	CMD_FLAG_BITMAP_OFS_POWERSAVE = 0,
	CMD_FLAG_BITMAP_OFS_FENCE = 1,
	CMD_FLAG_BITMAP_OFS_STATUS = 2,
	CMD_FLAG_BITMAP_OFS_FORCEDUAL = 62,
};
#define CMD_FLAG_BITMASK_STATUS 0xfULL
...
struct apusys_sc_hdr {
    ...
    unsigned int ofs_cb_dual0_info;
    unsigned int ofs_cb_dual1_info;
    unsigned int ofs_pmu_info;
} __attribute__((__packed__));

struct apusys_mdla_codebuf_info {
    unsigned int offset;
    ...
    unsigned int code_out_cmd_id;
} __attribute__((__packed__));

struct apusys_mdla_pmu_info {
    unsigned int ofs_result0;
    unsigned int ofs_result1;
    unsigned char mode;
    unsigned char num_events;
};

struct apusys_edma_codebuf_info {
    unsigned long long handle;
    unsigned int cmd_count;
    unsigned int addr;
    unsigned int fill_value;
    unsigned char desp_iommu_en;
} __attribute__((__packed__));
